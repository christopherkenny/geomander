...
A common issue in electoral geography is that data needs to be tied to a spatial component, but the pieces required for an analysis are not available at the desired levels of geography. This vignette walks through two common tasks. The first is collecting population data for precinct-level data, assuming that all (or nearly all) blocks can be matched to exactly one precinct. The second example is taking precinct-level data, estimating it down to a block level, and then agreggating up to voting districts. The second example is useful when state precincts do not match census voting districts or for when voting districts or precincts have changed over time.
...
One of the best sources of electoral data is the [Voting and Election Science Team](https://dataverse.harvard.edu/dataverse/electionscience) for spatial analysis. Using two counties from the Virginia senate election in 2018, this example walks through an easy way to add population data to this by matching based on the geographies.

First, we load in the two county subset included in the package.
```{r}
data(va18sub)
unique(va18sub$COUNTYFP)
```
After noting the counties included, we can use create_block_table to collect the key population boxes needed for most redistricting analysis for these two counties.

```{r, message = FALSE}
blocks87 <- create_block_table(state = 'VA', county = '087')  
blocks89 <- create_block_table(state = 'VA', county = '089')  
blocks <- bind_rows(blocks87, blocks89)
```
...
But proceeding with the default:
```{r}
matches <- geo_match(from = blocks, to = va18sub)
```

Then we can aggregate this up to the precinct level with block2prec.
```{r}
prec <- block2prec(block_table = blocks, matches = matches)
```

An alternative way to do this, especially when the edges of counties follow natural geographies like rivers, is to use the block2prec_by_county, which performs the above two steps, but only one county at a time. It returns the tables joined together.

```{r}
prec_by_c <- block2prec_by_county(block_table = blocks, precinct = va18sub, precinct_county_fips = 'COUNTYFP')
```
...
# Disaggregating and Reagreggating

A second common task is transfering information from one level of geography to another. In this case, I use the same Virginia county subset and blocks, but the end goal is to estimate electoral data in voting districts from known data in precincts.

The first dataset is the Virginia two county subset as above, but only looking at one county:
```{r}
data(va18sub)
...
```{r, message = FALSE}
blocks <- create_block_table(state = 'VA', county = '087')  
```

Third, we use tigris to get voting districts for the relevant Virginia counties.
```{r}
...
```{r}
vtd %>% filter(COUNTYFP10 == '087') %>% ggplot() +
  geom_sf() + 
...
```{r}
matches_p <- geo_match(from = blocks, to = va18sub)
```

And similarly, a vector of matches to voting district data (vtd) from blocks.
```{r}
matches_v <- geo_match(from = blocks, to = vtd)
```
...
```{r}
disagg_kaine <- estimate_down(wts = blocks$VAP, value = va18sub$G18USSDKAI, group = matches_p)
```
...
```{r}
disagg_kaine_geo <- geo_estimate_down(from = va18sub, to = blocks, wts = blocks$VAP, value = va18sub$G18USSDKAI)

vtd_kaine_geo <- geo_estimate_up(from = blocks, to = vtd, value = disagg_kaine_geo)
```
