...
```{r}
matches_p <- geo_match(from = block, to = va18sub)
```

And similarly, a vector of matches to voting district data (vtd) from blocks.
```{r}
matches_v <- geo_match(from = block, to = vtd)
```
...
```{r}
disagg_kaine_geo <- geo_estimate_down(from = va18sub, to = block, wts = block$vap, value = va18sub$G18USSDKAI)

vtd_kaine_geo <- geo_estimate_up(from = block, to = vtd, value = disagg_kaine_geo)
```
