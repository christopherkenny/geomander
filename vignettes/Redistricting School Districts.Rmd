...
comp <- redist.compactness(shp = blocks, district_membership = cds, adjacency = adj, measure = 'PolsbyPopper', ncores = 7)

comp_m <- comp %>% group_by(nloop) %>% summarize(mean = mean(PolsbyPopper), min = min(PolsbyPopper))
