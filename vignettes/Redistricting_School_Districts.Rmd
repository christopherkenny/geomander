...
```{r}
block <- block %>% geo_trim(to = school)
```
...
```{r}
matched <- geo_match(from = block, to = towns)
```
...
Finally, we have the data in a simulation-ready state! We can now use the adjacency list created above with redist.adjacency to run a simulation using redist.smc. See [the redist package](https://alarm-redist.github.io/) for more information about what's going on here.
```{r}
map <- redist_map(block, pop_tol = 0.01, ndists = 7, adj = adj)
...
```{r}
smry <- block2prec(block, matches = plans[,pick])
```
...
block2prec(block, matches = plans[,pick], geometry = TRUE) %>% 
  ggplot() + 
  geom_sf() + 
  theme_void() +
