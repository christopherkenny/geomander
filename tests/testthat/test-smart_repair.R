...
make_overlap_shp <- function() {
  r1 <- sf::st_polygon(list(matrix(
    c(0, 0, 1.5, 0, 1.5, 1, 0, 1, 0, 0), ncol = 2, byrow = TRUE
  )))
  r2 <- sf::st_polygon(list(matrix(
    c(0.5, 0, 2, 0, 2, 1, 0.5, 1, 0.5, 0), ncol = 2, byrow = TRUE
  )))
...
make_perfect_shp <- function() {
  ...
  sq3 <- sf::st_polygon(list(matrix(
    c(0, 1, 1, 1, 1, 2, 0, 2, 0, 1), ncol = 2, byrow = TRUE
  )))
  sq4 <- sf::st_polygon(list(matrix(
    c(1, 1, 2, 1, 2, 2, 1, 2, 1, 1), ncol = 2, byrow = TRUE
  )))
...
make_gap_shp <- function() {
  top <- sf::st_polygon(list(matrix(
    c(0, 0.55, 1, 0.55, 1, 1, 0, 1, 0, 0.55), ncol = 2, byrow = TRUE
  )))
  bottom <- sf::st_polygon(list(matrix(
    c(0, 0, 1, 0, 1, 0.45, 0, 0.45, 0, 0), ncol = 2, byrow = TRUE
  )))
  left <- sf::st_polygon(list(matrix(
    c(0, 0, 0.45, 0, 0.45, 1, 0, 1, 0, 0), ncol = 2, byrow = TRUE
  )))
  right <- sf::st_polygon(list(matrix(
    c(0.55, 0, 1, 0, 1, 1, 0.55, 1, 0.55, 0), ncol = 2, byrow = TRUE
  )))
...
make_pure_gap_shp <- function() {
  ...
  left <- sf::st_polygon(list(matrix(
    c(0, 0.45, 0.45, 0.45, 0.45, 0.55, 0, 0.55, 0, 0.45), ncol = 2, byrow = TRUE
  )))
  right <- sf::st_polygon(list(matrix(
    c(0.55, 0.45, 1, 0.45, 1, 0.55, 0.55, 0.55, 0.55, 0.45), ncol = 2, byrow = TRUE
  )))
  sf::st_sf(id = 1:4, geometry = sf::st_sfc(top, bottom, left, right))
}
...
test_that('smart_repair validates inputs and parameter ranges', {
  ...
  expect_error(
    smart_repair(sf::st_sf(id = 1, geometry = sf::st_sfc(sf::st_point(c(0, 0))))),
    'polygon'
  )
...
test_that('smart_repair resolves overlaps and preserves total area', {
  ...
  expect_true(all(as.character(sf::st_geometry_type(result)) %in%
                    c('POLYGON', 'MULTIPOLYGON')))

  # Overlap eliminated
  g <- sf::st_geometry(result)
  expect_true(as.numeric(sf::st_area(sf::st_intersection(g[[1]], g[[2]]))) < 1e-10)
...
test_that('smart_repair fills pure gap without shrinking any unit and splits among neighbors', {
  ...
  for (i in seq_len(4)) {
    expect_true(
      repaired_areas[i] >= original_areas[i] - 1e-10,
      label = sprintf('Unit %d shrank (was %.6f, now %.6f)',
                      i, original_areas[i], repaired_areas[i])
    )
  }
...
test_that('smart_repair handles convoluted input: overlaps resolved, gap filled, area preserved', {
  ...
  for (i in seq_len(n - 1L)) {
    for (j in seq(i + 1L, n)) {
      area <- as.numeric(sf::st_area(sf::st_intersection(g[[i]], g[[j]])))
      expect_true(
        area < 1e-8,
        label = sprintf('Overlap between units %d and %d: area = %.2e', i, j, area)
      )
    }
  }
  ...
  uncovered <- as.numeric(sf::st_area(
    sf::st_difference(gap_poly, sf::st_union(g))
  ))
  expect_true(uncovered < 1e-8,
  ...
  expect_true(result_area >= union_area - 1e-6,
              label = sprintf('Result area %.6f < union area %.6f', result_area, union_area))
})


# --- maup translations ---------------------------------------------------
# Translated from:
# https://github.com/mggg/maup/blob/master/tests/test_smart_repair.py
#
# The Python test suite uses two fixtures:
#   toy_precincts_geodataframe — a 4x4 grid of jagged polygons (random.seed(2023))
#   toy_counties_geodataframe  — a 2x2 grid of 1x1 unit squares over [0,2]x[0,2]
#
# Note: the Python suite also tests passing a bare GeoSeries; this is omitted
# because R's sf does not have a separate sfc-only entry point for smart_repair.
...
make_toy_precincts <- function() {
  ...
  for (i in 0:3) {
    for (j in 0:3) {
      bottom <- cbind(0.5 * i + 0.1 * (0:5),
                      0.5 * j + (runif(6) - 0.5) / 12)
      right  <- cbind(0.5 * (i + 1) + (runif(5) - 0.5) / 12,
                      0.5 * j + 0.1 * (1:5))
      top    <- cbind(0.5 * (i + 1) - 0.1 * (1:5),
                      0.5 * (j + 1) + (runif(5) - 0.5) / 12)
      left   <- cbind(0.5 * i + (runif(4) - 0.5) / 12,
                      0.5 * (j + 1) - 0.1 * (1:4))
...
make_toy_counties <- function() {
  mk <- function(x0, x1, y0, y1) {
    sf::st_polygon(list(matrix(
      c(x0, y0, x1, y0, x1, y1, x0, y1, x0, y0), ncol = 2, byrow = TRUE
    )))
  }
  sf::st_sf(
    id       = 1:4,
    geometry = sf::st_sfc(mk(0, 1, 0, 1), mk(1, 2, 0, 1),
                          mk(0, 1, 1, 2), mk(1, 2, 1, 2))
  )
}
...
test_that('smart_repair [maup]: basic output is a clean sf (no overlaps, valid geometries)', {
...
test_that('smart_repair [maup]: nest_within_regions keeps each precinct inside its county', {
  precincts <- make_toy_precincts()
  ...
  result <- smart_repair(precincts, regions = counties,
                         region_col = 'county_id', epsg = FALSE)

  for (p in seq_len(nrow(result))) {
    c_idx      <- precincts$county_id[p]
    diff_area  <- as.numeric(sf::st_area(
      sf::st_difference(sf::st_geometry(result)[[p]],
                        sf::st_geometry(counties)[[c_idx]])
    ))
    expect_true(
      diff_area < 1e-8,
      label = sprintf('Precinct %d extends outside county %d by area %.2e',
                      p, c_idx, diff_area)
    )
  }
})

test_that('smart_repair [maup]: rook_threshold converts short rook adjacencies to queen', {
  precincts    <- make_toy_precincts()
