...
create_block_table <- function(state, county = NULL, geometry = TRUE, year = 2020, mem = FALSE, 
  ...
  if (mem) {
    out <- censable::mem_build_dec(geography = 'block', state = state, county = county, 
                                   geometry = geometry, year = year, groups = 'all')
  } else {
    out <- censable::build_dec(geography = 'block', state = state, county = county, 
                               geometry = geometry, year = year, groups = 'all')
  }
...
create_tract_table <- function(state, county, geometry = TRUE, year = 2019,
  ...
  if (mem){
    out <- censable::mem_build_acs(geography = 'tract', state = state, county = county, 
                                   geometry = geometry, year = year, groups = 'all')
  } else{
    out <- censable::build_acs(geography = 'tract', state = state, county = county, 
                               geometry = geometry, year = year, groups = 'all')
  }
